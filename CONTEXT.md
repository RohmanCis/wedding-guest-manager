...
- **sort state** — the active guest-table ordering: `SortState { key, dir }` with `SortKey ∈ {name, pax, party_name, group_name}`. Client-side and instant (`sortGuests()` in `src/lib/guest-sort.ts`, pure, pax numeric, `id` locale, name-asc tiebreak); the CSV export mirrors it server-side through the filter seam (`sort`/`dir` params → whitelisted `SORT_COLUMNS` ORDER BY). Default `{name, asc}` equals the old fixed server order — zero visual diff. Not URL-synced, not multi-column.
...
- **section** — the grouping unit of `/daftar`: one Party or one Group, rendered as a sticky header (dot + icon + name + entry count) followed by that category's guests in `name asc` order, sections ordered by the category list order. Section counts are **entries, not people** (never pax-weighted — same rule as the analytics distribution), and sections always hide when empty. Pure grouping core: `groupGuests()` in `src/lib/group-guests.ts`.
