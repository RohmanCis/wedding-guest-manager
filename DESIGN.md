...
**Sortable headers.** The four data columns (Nama, Jumlah, Party, Group) sort on click; `No.` and Alamat never do. Each header is a full-cell ghost button inside the `TableHead` — hit area ≥44px via negative-margin padding (`-m-3 p-3 min-h-11`) — carrying `aria-sort` (`ascending`/`descending`/`none`) on the `<th>` for screen readers. Direction icons: idle `ChevronsUpDown` in Muted, invisible until header hover/focus on pointer devices (`group-hover/head`) but always faintly visible (40% opacity) below `lg` where touch has no hover; the active column swaps to `ChevronUp`/`ChevronDown` in Wedding Gold. Icon width is space-reserved so toggling never shifts column widths; Party/Group headers stay width-unconstrained. Sorting reorders instantly client-side (no refetch) and feeds every index-based consumer (page slice, duplicate-jump math, row numbering). While a category column (Party/Group) drives the sort, adjacent rows whose category value changes get a `border-default` top separator — a quiet rule that keeps custom-category fallback colors from visually colliding across runs.
