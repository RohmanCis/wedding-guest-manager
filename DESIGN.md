...
**Sortable headers.** The four data columns (Nama, Jumlah, Party, Group) sort on click; `No.` and Alamat never do. Each header is a full-cell ghost button inside the `TableHead` — hit area ≥44px via negative-margin padding (`-m-3 p-3 min-h-11`) — carrying `aria-sort` (`ascending`/`descending`/`none`) on the `<th>`, an `aria-label`/`title` of "Urutkan berdasarkan {label}", and a gold focus ring offset against `surface-1` (the sticky header surface). Icons: inactive columns always show `ArrowUpDown` in Muted at 70% opacity (≥3:1 on the header surface) at every breakpoint, rising to full opacity on header hover/focus-within; the active column swaps to `ArrowUp`/`ArrowDown` in Wedding Gold and lifts its label to Primary Text (inactive labels keep the ghost button's Secondary Text). Icon width is space-reserved so toggling never shifts column widths; Party/Group headers stay width-unconstrained. Sorting reorders instantly client-side (no refetch) and feeds every index-based consumer (page slice, duplicate-jump math, row numbering). Every toggle also announces the new order through an sr-only `aria-live="polite"` region ("Terurut berdasarkan {Kolom}, menaik/menurun") that renders empty on first paint and never announces on initial render. While a category column (Party/Group) drives the sort, adjacent rows whose category value changes get a `border-default` top separator — a quiet rule that keeps custom-category fallback colors from visually colliding across runs.

On mobile the guest view adapts: the filter toolbar stacks vertically with full-width search and selects (`sm` restores the inline row), Export CSV and Tambah Tamu move into a sticky action bar pinned directly above the bottom nav (`bottom-14`, visible below `sm` only — `sm+` shows them in the top bar), the Alamat column hides below `sm`, and row actions stay always-visible as 32×32 touch targets. On desktop the Tambah Tamu button also opens via Ctrl/Cmd+Enter: a static `<kbd>Ctrl ↵</kbd>` hint (aria-hidden, `hidden lg:inline-flex` so it never renders on touch or below `lg`) sits inside the existing gold button, and `aria-keyshortcuts="Control+Enter Meta+Enter"` advertises the accelerator; the Nama field autofocuses on open for fine pointers only.

### Pagination
The guest table paginates at 10 rows per page. Inactive page links and prev/next buttons use the ghost Button tokens (Secondary Text warming to Primary on a Surface 3 wash); the active link carries the gold identity — `accent-gold-subtle` wash, `border-accent-gold`, Warm Cream text — and `aria-current="page"`. Ellipses and the "Menampilkan X–Y dari Z tamu" count line sit in Muted `text-xs` (count-in-muted is an explicit product decision for this line only). When the sort is not the default (`name asc`), that same line carries the active order — `Menampilkan 1–10 dari 311 tamu · Terurut: Party (Z–A) · Urutan default`, with text columns reading `A–Z`/`Z–A` and Jumlah reading `1→4`/`4→1` (wording resolves from `guest-sort.ts`, never inlined) — and a ghost `sm` **Urutan default** button resetting to `DEFAULT_SORT`; at the default state the line is unchanged and the reset control is absent. Wording wraps naturally, so the line stays intact at 375px. Controls appear below the table only when there is more than one page; desktop shows 7 page items, mobile 5 (via `useIsMobile`). A "Tampilkan Semua" ghost `sm` toggle sits beside the controls and renders the unfiltered-by-pagination full list (label swaps to "Tampilkan Per Halaman", which returns to page 1); any filter or sort change resets to page 1 in paginated mode. Page changes animate the old rows out (y −8, 150ms) and the new rows in (y 8→0, 200ms) under `AnimatePresence`, zeroed for reduced motion.

### Sectioned List
Read-only scan surface on `/daftar`. One scrolling page, no pagination: sections follow the category list order, each opened by a sticky header (`sticky top-12` under the 48px top bar, Surface 1, 40px tall, bottom default border, z below the top bar) carrying the category identity — color dot with the CategoryIcon beside it, Figtree medium name, and a right-aligned Secondary Text tabular entry count ("N tamu"; counts are entries, never pax-weighted, and never Muted — counts are data). Rows are list markup, not tables: name (medium, truncated) over address (xs, Secondary, truncated), with `×N` pax in Secondary tabular only when pax > 1. Empty sections never render. The mode toggle (Per Pihak / Per Grup) reuses the analytics wording and the secondary/ghost button pair; the filter toolbar mirrors the guests-view stacking (full-width below `sm`). No row motion and no primary action — a pure reading surface; filtering dims the mounted rows (`aria-busy` + opacity) rather than swapping in skeletons.
